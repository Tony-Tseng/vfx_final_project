...
def pixel_consistency(prime_frame, second_frame, pts_prime, pts_second, window_size=3):
    ...
    height, width = I_second.shape

    kernel = np.ones((window_size, window_size),np.uint8)
    I_second_max = cv2.dilate(I_second, kernel, iterations=1)
    I_second_min = cv2.erode(I_second, kernel, iterations=1)

    # distribution
    rv = scipy.stats.norm(loc=0, scale=4)
    ...
    for i, (pt_prime, pt_second) in enumerate(zip(pts_prime, pts_second)):
        ...
        pixel_prob = rv.pdf(pixel_loss) * 2
        pixel_consistency_prob.append(pixel_prob)
...
def calculate_whole_prob(second_frame, pts_second, pixel_prob, motion_prob):
    ...
    # prob_frame = np.array( [np.tile(np.arange(width),height), np.tile(np.arange(height),width)] ).T
    # prob_frame = prob_frame.dot(theta).reshape(width, height).T
...
def wm(point, X, distance_mean):
    ...
    for i in range(m): 
        xi = X[i] 
        d = (-2 * distance_mean[i] * distance_mean[i]) 
